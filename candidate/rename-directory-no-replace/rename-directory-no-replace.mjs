...
import { loadNativeAddon } from "./loader.mjs";
...
/** Mechanically freezes one sibling-directory publication candidate. */
...
/** Candidate operation. It never falls back to a JavaScript exists+rename sequence. */
export async function publishFixedSet({ sourceRoot, targetParent, targetSegment, manifest } = {}) {
  ...
  try {
    ...
    return invalidReceipt(manifest, "UNSUPPORTED_PLATFORM", cause?.message ?? "native candidate unavailable");
  }
